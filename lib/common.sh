...
install_codex_config() {
  ...
  local reasoning_effort="${CODEX_REASONING_EFFORT:-minimal}"
...
model_reasoning_effort = $(_toml_quote "$reasoning_effort")
...
EOF
  ...
  echo "[exe-setup] codex config: $config_file (model=$model, provider=$provider_name, base_url=$base_url, wire_api=$wire_api, service_tier=$service_tier, reasoning_effort=$reasoning_effort)"
}
